 class   Movie  < ApplicationRecord
  ...
  validates :category, presence: true, if:  ->{ serie.nil? } validate :highlight_episode

  private

  def highlight_episode
    if self.serie.present? && self.highlighted == true errors.add( :highlight_episode, "It's not possible to highlight
an serie episode" )
    end
  end
end
